...
kable(tab,
      format = "latex", linesep = "", align='c', booktabs = T, row.names = FALSE, longtable = TRUE, caption = 'Table A. Genes annotated as EAL.') %>%
      kable_styling(latex_options = c("scale_down", "hold_position", "repeat_header"), font_size = 10)
```
...
Table B summarizes the overlap of integrated genes, categorized with correction for multiple comparisons, and the different oncogene lists. 
Significance of enrichment was tested using Fisher's Exact test of proportions. 

\vspace{0.25cm}

```{r, oncoGeneTable, echo=FALSE, message=FALSE, warning=FALSE}
kable(correctedGeneListTests, format = "latex", align='lcccl', linesep = c('', '', '', '\\hline'), booktabs = T, row.names = FALSE, caption = 'Table B2.') %>%
...
threshold for inclusion is an estimated abundances $\geq$ `r config$minAbundCatThreshold` cells.
...
frequency post-transfusion and *fo* is the integration of pre-infused transduction material. The significance of these 
changes is assessed using Fisher's Exact tests corrected for multiple comparisons. A similar plot using uncorrected 
p-values can be found at the send of this report (Figure S1).
...
Figure S1. Volcano plot using uncorrected Fisher Exact test p-values.  
